...
class TestToImagePlugin(unittest.TestCase):
    ...
    def test_supported_formats(self):
        ...
        self.assertEqual(sorted(self.converter.SUPPORTED_FORMATS), sorted(expected_formats))
    ...
    not os.path.exists(
    os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "test_sample.pdf"
    )
    ),
    ...
    def test_convert_png_format(self):
        """测试转换为 PNG 格式"""
        result = self.converter.convert(self.test_pdf_path, format="png")
    ...
    def test_convert_jpeg_format(self):
        """测试转换为 JPEG 格式"""
        result = self.converter.convert(self.test_pdf_path, format="jpeg")
    ...
    def test_convert_jpg_format(self):
        ...
        result = self.converter.convert(self.test_pdf_path, format="jpg")
    ...
    def test_convert_pnm_format(self):
        """测试转换为 PNM 格式"""
        result = self.converter.convert(self.test_pdf_path, format="pnm")
    ...
    def test_convert_unsupported_format(self):
        """测试不支持的格式"""
        result = self.converter.convert(self.test_pdf_path, format="gif")
    ...
    def test_convert_with_pages_list(self):
        """测试转换指定页码列表"""
        result = self.converter.convert(self.test_pdf_path, pages=[1, 2])
    ...
    def test_convert_with_dpi(self):
        """测试设置 DPI"""
        result = self.converter.convert(self.test_pdf_path, dpi=300)
    ...
    def test_convert_with_quality(self):
        """测试设置图片质量"""
        result = self.converter.convert(self.test_pdf_path, quality=95)
    ...
    def test_convert_with_grayscale(self):
        """测试灰度转换"""
        result = self.converter.convert(self.test_pdf_path, grayscale=True, format="png")
    ...
    def test_convert_with_embed(self):
        """测试嵌入 base64 数据"""
        result = self.converter.convert(self.test_pdf_path, embed=True, page=1)
    ...
    def test_convert_high_dpi(self):
        """测试高 DPI 转换"""
        result = self.converter.convert(self.test_pdf_path, dpi=600, page=1)
    ...
    def test_convert_low_dpi(self):
        """测试低 DPI 转换"""
        result = self.converter.convert(self.test_pdf_path, dpi=72, page=1)
    ...
    def test_determine_pages_to_process_single_page(self):
        """测试确定处理页码 - 单页"""
        kwargs = {"page": 3}
        pages = self.converter._determine_pages_to_process(kwargs, 10)
    ...
    def test_validate_input(self):
        ...
        # 无效输入 - 不支持的格式
        self.assertFalse(self.converter.validate_input(pdf_path="test.pdf", format="gif"))

        # 无效输入 - 无效 DPI
        self.assertFalse(self.converter.validate_input(pdf_path="test.pdf", dpi=-1))

        # 无效输入 - 无效质量
        self.assertFalse(self.converter.validate_input(pdf_path="test.pdf", quality=150))
    ...
    def test_convert_multiple_formats(self):
        ...
        for fmt in formats:
            with self.subTest(format=fmt):
                result = self.converter.convert(self.test_pdf_path, format=fmt, page=1)
    ...
    def test_convert_with_embed_all_pages(self):
        """测试嵌入所有页面"""
        result = self.converter.convert(self.test_pdf_path, embed=True)
    ...
    def test_convert_quality_clamping(self):
        """测试质量参数限制"""
        # 测试高质量
        result = self.converter.convert(self.test_pdf_path, quality=150, page=1)
        self.assertTrue(result["success"])

        # 测试低质量
        result = self.converter.convert(self.test_pdf_path, quality=-10, page=1)
        self.assertTrue(result["success"])
    ...
    def test_convert_different_dpi_resolutions(self):
        """测试不同 DPI 分辨率"""
        dpi_values = [72, 150, 300, 600]

        for dpi in dpi_values:
            with self.subTest(dpi=dpi):
                result = self.converter.convert(self.test_pdf_path, dpi=dpi, page=1)

                self.assertTrue(result["success"], f"Failed to convert with DPI {dpi}")
                self.assertEqual(result["images"][0]["dpi"], dpi)
    ...
    def test_convert_with_grayscale_and_embed(self):
        """测试灰度转换同时嵌入 base64"""
        result = self.converter.convert(
            self.test_pdf_path,
            grayscale=True,
