...
@click.option('-o', '--output', type=click.Path(), required=True, help='Output Markdown file')
...
@click.option('--preserve-tables', is_flag=True, default=True, help='Preserve tables in Markdown')
@click.option('--preserve-images', is_flag=True, default=True, help='Preserve images in Markdown')
@click.option('--image-prefix', type=str, default='image_', help='Image filename prefix')
@click.pass_context
def to_markdown_command(ctx, input, output, page, page_range, pages, preserve_tables, preserve_images, image_prefix):
    ...
    if not plugin.is_available():
        click.echo("Error: ToMarkdown plugin is not available. Check dependencies.", err=True)
        return
