...
@click.option(
    "-o", "--output", type=click.Path(), required=True, help="Output HTML file"
)
...
@click.option(
    "--embed-images", is_flag=True, default=False, help="Embed images as base64 in HTML"
)
@click.option(
    "--responsive", is_flag=True, default=False, help="Use responsive CSS design"
)
@click.pass_context
def to_html_command(
    ctx, input, output, page, page_range, pages, embed_images, responsive
):
    ...
    if not plugin.is_available():
        click.echo(
            "Error: ToHtml plugin is not available. Check dependencies.", err=True
        )
        return
