...
@click.option(
    "-o", "--output", type=click.Path(), required=True, help="Output EPUB file"
)
...
@click.option(
    ...
    default=True,
    help="Include images in EPUB (default: True)",
)
@click.option(
    "--chapter-pages",
    type=int,
    default=0,
    help="Pages per chapter (0 = one chapter per page, default: 0)",
)
@click.pass_context
def to_epub_command(
    ctx,
    ...
    if not plugin.is_available():
        click.echo(
            "Error: ToEpub plugin is not available. Check dependencies.", err=True
        )
        return
