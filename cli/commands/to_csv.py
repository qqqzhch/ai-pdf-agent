...
@click.option(
    "-o", "--output", type=click.Path(), required=True, help="Output CSV file"
)
...
@click.option(
    ...
    type=int,
    default=0,
    help="Table index to convert (0-based, default: 0)",
)
@click.option(
    "--merge-tables", is_flag=True, default=False, help="Merge all tables into one CSV"
)
@click.option(
    "--header/--no-header", default=True, help="Include table header (default: True)"
)
...
def to_csv_command(
    ...
    if not plugin.is_available():
        click.echo(
            "Error: ToCsv plugin is not available. Check dependencies.", err=True
        )
        return
