...
@click.option(
    "-o", "--output", type=click.Path(), help="Output file path (save to file)"
)
...
@click.option(
    ...
    "--format",
    "output_format",
    type=click.Choice(["json", "csv", "list"], case_sensitive=False),
    default="json",
...
def tables_command(ctx, input, output, pages, output_format, csv_output):
    ...
    if not plugin.is_available():
        raise click.ClickException("Error: Table reader plugin is not available")
    ...
    if output and output_format.lower() == "json":
        click.echo(
            f"Summary: {result['total_tables']} tables found on {len(result['pages_extracted'])} page(s)"
        )
