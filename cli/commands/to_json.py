...
@click.option(
    "-o", "--output", type=click.Path(), required=True, help="Output JSON file"
)
...
@click.option(
    "--structured", is_flag=True, default=True, help="Structured JSON format (default)"
)
@click.option(
    "--include-metadata", is_flag=True, default=True, help="Include document metadata"
)
...
@click.option(
    "--include-structure", is_flag=True, default=True, help="Include document structure"
)
@click.pass_context
def to_json_command(
    ctx,
    ...
    if not plugin.is_available():
        click.echo(
            "Error: ToJson plugin is not available. Check dependencies.", err=True
        )
        return
