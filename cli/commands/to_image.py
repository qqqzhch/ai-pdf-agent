...
@click.option(
    ...
    type=click.Path(),
    required=True,
    help='Output directory or file template (e.g., "output/" or "page_{page}.png")',
)
...
def to_image_command(
    ...
    if dpi < 72 or dpi > 600:
        click.echo(
            f"Warning: DPI {dpi} is outside recommended range (72-600)", err=True
        )
    ...
    if not plugin.is_available():
        click.echo(
            "Error: ToImage plugin is not available. Check dependencies.", err=True
        )
        return
