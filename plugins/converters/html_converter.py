"""HTML 转换器插件 - 使用 PyMuPDF 将 PDF 转换为 HTML

这个模块实现了 ToHtmlConverter 类，用于将 PDF 文档转换为 HTML 格式。
支持文本、标题、表格、列表和图片的转换，并提供可自定义的 CSS 样式。
"""
...
class ToHtmlConverter(BaseConverterPlugin):
    """HTML 转换器 - 将 PDF 内容转换为 HTML 格式

    这个类使用 PyMuPDF 引擎提取 PDF 内容，并将其转换为结构化的 HTML 文档。
    支持的功能包括：
    - 文本和段落转换
    - 标题识别（基于字体大小）
    - 列表转换（有序和无序列表）
    ...
    def convert(self, pdf_path: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            custom_css = kwargs.get("custom_css", None)
            ...
            html_content = self._build_html_document(
                doc, pages_to_convert, embed_images, responsive, custom_css
            )
    ...
    def _build_html_document(
        ...
        # 默认样式
        html_parts.append("        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }")
        html_parts.append("        .page { margin: 20px 0; padding: 20px; border: 1px solid #ddd; }")
        html_parts.append("        h1, h2, h3, h4 { color: #2c3e50; margin-top: 20px; }")
        html_parts.append("        p { margin: 10px 0; }")
        html_parts.append("        li { margin: 5px 0; }")
        html_parts.append("        ul, ol { margin: 10px 0; padding-left: 20px; }")

        # 响应式样式
        if responsive:
            html_parts.append("        @media (max-width: 768px) {")
            html_parts.append("            body { padding: 10px; font-size: 14px; }")
            html_parts.append("            .page { padding: 10px; }")
            html_parts.append("        }")
            html_parts.append("        @media (min-width: 769px) {")
            html_parts.append("            body { max-width: 1200px; margin: 0 auto; padding: 40px; }")
            html_parts.append("        }")

        # 自定义样式
        if custom_css:
            html_parts.append(f"        {custom_css}")
