...
def format_benchmark_results(results: List[BenchmarkResult]) -> str:
    ...
    # 表头
    lines.append(f"{'Name':<30} {'Iterations':<12} {'Avg Time':<12} {'Min Time':<12} {'Max Time':<12} {'Ops/sec':<12}")
    lines.append("-" * 80)
...
def format_comparison_results(results: List[BenchmarkResult]) -> str:
    ...
    for result in results[1:]:
        ...
        lines.append(f"  Improvement: {(1 - result.avg_time/baseline.avg_time)*100:.1f}%")
        lines.append("")
