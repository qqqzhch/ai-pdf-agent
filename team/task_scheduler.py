...
class DynamicTaskScheduler:
    ...
    def _create_initial_state(self) -> dict:
        ...
        return {
            ...
            "task_list": [],
            "active_agents": [],
            "execution_plan": {"groups": [], "current_group_index": 0},
            "metrics": {"start_time": None, "end_time": None, "total_duration_ms": 0},
            "last_update": None,
        }
    ...
    def _update_task_state(self):
        ...
        pending = len(
            [t for t in self.tasks if self._get_task_status(t["id"]) == "pending"]
        )
        in_progress = len(
            [t for t in self.tasks if self._get_task_status(t["id"]) == "in_progress"]
        )
        completed = len(
            [t for t in self.tasks if self._get_task_status(t["id"]) == "completed"]
        )
        failed = len(
            [t for t in self.tasks if self._get_task_status(t["id"]) == "failed"]
        )
    ...
    def plan_execution(self) -> List[List[dict]]:
        ...
        # 3. 保存执行计划
        self.state["execution_plan"] = {"groups": groups, "current_group_index": 0}
    ...
    def _identify_parallel_groups(self, tasks: List[dict]) -> List[List[dict]]:
        ...
        while remaining_tasks:
            ...
            remaining_tasks = [
                t for t in remaining_tasks if t["id"] not in used_task_ids
            ]
    ...
    def _execute_task(self, task: dict):
        ...
        session_key = self.agent_pool.spawn_agent(
            role="developer", task=task, timeout=task.get("timeout", 3600)
        )
        ...
        if session_key:
            ...
            qa_session = self.agent_pool.spawn_agent(
                role="quality_engineer", task=task, timeout=600
            )
